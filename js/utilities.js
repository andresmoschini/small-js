...
export var reverseColorNames = {};

for (var name in colorNames) {
	if (colorNames.hasOwnProperty(name)) {
		reverseColorNames[colorNames[name]] = name;
	}
}
...
export function colorToRgb(color) {
	...
	} else if (match = color.match(abbr)) {
		match = match[1];
		hexAlpha = match[3];
	...
	} else if (match = color.match(keyword)) {
...
export function rgbToKeyword(rgb) {
	return reverseColorNames[rgb.slice(0, 3)];
};
